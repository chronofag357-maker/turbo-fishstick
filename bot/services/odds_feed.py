"""Server-only bookmaker feed; a 96-hour shared cache protects the quota."""
import asyncio
import json
import time
...
async def get_feed(sport, force=False, event_id=None):
    async with LOCK:
        ...
        if not force and old and old.get('regions') == REGIONS and time.time() - old['fetched_at'] < REFRESH_HOURS * 3600:
            return {**old, 'refresh_hours': REFRESH_HOURS, 'stale': False}
        try:
            ...
            result = {'events': rows, 'fetched_at': time.time(), 'source': 'The Odds API', 'refresh_hours': REFRESH_HOURS, 'regions': REGIONS}
            if event_id:
