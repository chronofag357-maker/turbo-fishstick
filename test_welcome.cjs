...
(async()=>{
 ...
 try{for(const width of [320,390,1280]){
  ...
  await page.locator('[data-verify-slider]').focus();for(let i=0;i<12;i++)await page.keyboard.press('ArrowLeft');
  assert.equal(await page.locator('.auth-slide').evaluate(el=>el.style.getPropertyValue('--auth-progress')),'60%');
  await page.screenshot({path:`.tools/welcome-slider-${width}.png`});
  assert(await page.locator('.auth-slide').isVisible());
  await page.locator('[data-verify-slider]').focus();
  await page.keyboard.press('End');
  assert.equal(await page.locator('[data-verify-slider]').getAttribute('aria-valuenow'),'100');
