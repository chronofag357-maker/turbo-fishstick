...
(async()=>{
 ...
 try {
  for(const width of [390,320]){
   ...
   assert.deepEqual(await page.locator('.es-teams strong').first().evaluate(e=>({color:getComputedStyle(e).color,weight:getComputedStyle(e).fontWeight})),{color:'rgb(32, 36, 63)',weight:'600'});
   const match=await page.locator('.es-match').boundingBox(),prices=await page.locator('.es-prices').boundingBox();
   assert(Math.abs(prices.width/match.width-.433)<.005,'Same names/odds split as combat cards');
   assert(match.height<65,'Compact standard two-team row');
   assert.equal(await page.locator('.es-title').evaluate(e=>getComputedStyle(e).minHeight),'44px');
   assert.equal(await page.locator('.es-team-open').evaluate(e=>getComputedStyle(e).fontSize),'12px');
   await page.screenshot({path:'.tools/esports-compact-'+width+'.png'});
   const originalName=await page.locator('.es-teams strong').first().textContent();
   await page.locator('.es-teams strong').first().evaluate(e=>e.textContent='Очень длинное название киберспортивной команды International Championship Academy');
   assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Long names do not widen page');
   assert(await page.locator('.es-teams strong').first().evaluate(e=>e.scrollWidth<=e.clientWidth&&e.scrollHeight<=e.clientHeight),'Long name not clipped');
   await page.locator('.es-teams strong').first().evaluate((e,name)=>e.textContent=name,originalName);
   ...
   await page.click('[data-es-expand]');assert.equal(await page.locator('#event-detail').isVisible(),true);
   ...
   let ready=false;
   await page.route('**/api/esports-live',r=>r.abort());
   ...
   ready=true;await page.click('[data-es-refresh]');await page.click('[data-es-refresh]');
