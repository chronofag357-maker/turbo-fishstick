...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 for(const width of [320,390,450]){
  ...
  assert.equal(check.count,5);assert.equal(check.overflow,false);assert.equal(check.page,false);assert.equal(check.visible,true);assert(check.header<=145,JSON.stringify(check));
