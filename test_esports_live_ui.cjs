...
(async()=>{
 ...
 try{for(const width of [320,390]){
  ...
  const arrow=await p.locator('.es-up .es-move svg').first().evaluate(e=>({height:e.getBoundingClientRect().height,stroke:getComputedStyle(e).strokeWidth}));
  assert.equal(arrow.height,28);assert.equal(arrow.stroke,'2.2px');
  await p.screenshot({path:'.tools/esports-arrows-'+width+'.png'});
