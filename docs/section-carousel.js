...
(() => {
  ...
  function leave(){
    if(!selected)return;
    window.TradingTerminal?.close();selected='';app.classList.remove('extra-section-open');screen.hidden=true;
  ...
  nav.addEventListener('click',e=>{
    ...
    window.TradingTerminal?.close();screen.replaceChildren();
    if(selected==='trading')window.TradingTerminal?.open(screen);
    if(selected==='p2p')window.PredictionMarket?.open(screen);
    nav.querySelectorAll('.sport').forEach(n=>{n.classList.toggle('active',n===b);n.setAttribute('aria-pressed',String(n===b));});
  },true);
  nav.classList.add('section-carousel');nav.setAttribute('aria-label','Разделы');
