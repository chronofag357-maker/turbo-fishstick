...
(() => {
  ...
  dialog.addEventListener('submit',async e=>{
    ...
    if(!confirm('Подтвердить результат? Связанные купоны будут рассчитаны, выплаты поступят на баланс.'))return;
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(button.hasAttribute('data-server-ledger')){
        shell('История операций');await refresh();dialog.querySelector('[data-admin-body]').innerHTML=(current?.transactions||[]).map(t=>`<p>${escape(new Date(t.created*1000).toLocaleString('ru-RU'))} · ${escape(t.kind)}<br>${t.amount>0?'+':''}${money(t.amount/100)} · ${escape(t.bet_id||'Начальный баланс')}</p>`).join('');return;
      }
