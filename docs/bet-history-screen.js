...
(() => {
  ...
  function paint(){
    ...
    for(const bet of bets){
      ...
      const note=card.querySelector('.bet-card-body>.receipt-meta');if(note)note.textContent=state(bet)==='pending'?'Учебное пари · ожидает результата.':'Учебное пари · результат из записи.';
  ...
  screen.addEventListener('click',e=>{if(e.target.closest('[data-history-menu]'))panel('Меню',window.freebkMenuContent());const t=e.target.closest('[data-history-tab]');if(t){tab=t.dataset.historyTab;paint();screen.scrollTop=0}if(e.target.closest('[data-history-settings]')){const f=screen.querySelector('.history-filters');f.hidden=!f.hidden}if(e.target.closest('[data-history-fund]'))screen.querySelector('.history-message').textContent='Пополнение пока не подключено. Используется тестовый баланс.'});
