...
(() => {
  ...
  async function exportAction(e,b,container){
    ...
    if(e.target.closest('[data-receipt-share]')){
      try{if(navigator.share)await navigator.share({title:'P2P Market — купон',text:text(b)});else{await navigator.clipboard.writeText(text(b));status.textContent='Купон скопирован — вставьте его в сообщение или письмо.'}}catch(err){if(err.name!=='AbortError')status.textContent='Отправка недоступна. Используйте «Скачать» или «Почта».'}return true;
    }
    return false;
  }
  ...
  window.createHistoryReceipt=bet=>{
    ...
    const note=document.createElement('p');note.className='receipt-meta';note.textContent=bet.status&&bet.status!=='pending'?'Пари · рассчитано.':'Пари · результат ещё не рассчитан.';body.append(note);
